...
<div class="container" style="width: 600px">
    <div class="d-flex mt-3 border position-relative" style="height: 120px">
        ...
        <a href="{{ route('item_get', ['id'=>$value->id]) }}" class="stretched-link"></a>
    </div>    
</div>
{{-- ステータス --}}
<div class="container" style="width: 600px">
    <div class="pt-3" style="font-size: 18px">①発送→②配達中→③購入者評価→取引終了</div>
    <div class="font-weight-bold text-center border-bottom pb-3" style="font-size: 22px">
        @if($channel->status == 2)
            <br>支払いが完了しました<br>
            発送してください
            <form action="{{ route('status', ['id'=>$channel->id]) }}" method="post">
                @csrf
        ...
        @foreach ($messages as $key=>$value)
            <div class="wrapper row">
                ...
                @if($value->owner_id == $user->id)
                    <label for="">
                        <img src="{{ $user->avatar }}" class="rounded-circle" style="width: 2rem" alt="...">
                        {{ $user->nickname }}
                    </label>
                {{-- 出品者 --}}
                @elseif($value->owner_id == Auth::user()->id)
                    <label for="">
